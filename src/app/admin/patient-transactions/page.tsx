...
export default function PatientTransactionsPage() {
  ...
  useEffect(() => { load(); }, []);
  const load = async () => {
    try { setLoading(true); const { data } = await supabase.from("patient_transactions").select("*").order("created_at", { ascending: false }); setTxns(data || []); } catch (e) { console.error(e); } finally { setLoading(false); }
  };
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center gap-4">
        ...
        <div><h1 className="text-2xl font-bold">Patient Transactions</h1><p className="text-sm text-muted-foreground">{filtered.length} of {txns.length} transactions</p></div>
      </div>
