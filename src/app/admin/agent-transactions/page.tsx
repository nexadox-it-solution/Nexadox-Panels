...
export default function AgentTransactionsPage() {
  ...
  const load = async () => {
    try {
      setLoading(true);
      const { data } = await supabase.from("agent_transactions").select("*").order("created_at", { ascending: false });
      const rows = data || [];
      // Enrich with profile name/email where missing
      const ids = [...new Set(rows.filter(t => t.user_id && (!t.user_name || !t.user_email)).map(t => t.user_id))];
      if (ids.length > 0) {
        const { data: profiles } = await supabase.from("profiles").select("id, name, email").in("id", ids);
        const pm = new Map((profiles || []).map((p: any) => [p.id, p]));
        rows.forEach(t => { const p = pm.get(t.user_id); if (p) { if (!t.user_name) t.user_name = p.name || "—"; if (!t.user_email) t.user_email = p.email || "—"; } });
      }
      setTxns(rows);
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center gap-4">
        ...
        <div><h1 className="text-2xl font-bold">Agent Transactions</h1><p className="text-sm text-muted-foreground">{filtered.length} of {txns.length} transactions</p></div>
      </div>
